...
def GAC_active_contour(x, y, input_path, BWoutput_pathGAC):
    ...
    image_u.save(BWoutput_pathGAC + '.jpg')
...
def CV_active_contour(x, y, input_path, BWoutput_pathCV):
    ...
    #Save masks
    image_u = Image.fromarray((array * 255).astype(np.uint8), mode='L')
    image_u.save(BWoutput_pathCV + '.jpg')
    
def pipLine(center, filename):
    logging.basicConfig(level=logging.DEBUG)

    input_path = 'YOLO/' + filename + "_1_crop_F.jpg"
    BWoutput_pathCV = 'YOLO/mask_CVBWCF_' + '_' + filename
    BWoutput_pathGAC = 'YOLO/mask_GACBWCF_' + '_' + filename

    GAC_active_contour(center[0], center[1], input_path, BWoutput_pathGAC)
    CV_active_contour(center[0], center[1], input_path, BWoutput_pathCV)
